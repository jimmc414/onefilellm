...
Test Name: alias-with-args
Generated: 2025-06-12 15:57:16 UTC
Command: python onefilellm.py test-repo --crawl-max-depth 1

Recorded test for: test-repo --crawl-max-depth 1
...
@pytest.mark.unit
@pytest.mark.alias
class TestRecordedAlias_With_Args:
    """Auto-generated test class for recorded command execution."""
    
    def test_alias_with_args(self, snapshot):
        """
        Recorded test for: test-repo --crawl-max-depth 1
        
        This test captures the exact output of running:
        python onefilellm.py test-repo --crawl-max-depth 1
        """
        # Execute the command that was recorded
        args = ['test-repo', '--crawl-max-depth', '1']
        
        # Run the program and capture output
        output = run_program(args)
        
        # Assert against the recorded snapshot
        snapshot.assert_match(output, "alias_with_args_f558c874")
    
    def test_alias_with_args_success_expectation(self):
        """
